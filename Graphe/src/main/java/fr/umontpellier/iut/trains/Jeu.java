...
public class Jeu implements Runnable {
    ...
    public Graphe getGraphe() {
        Set<Sommet> SommetsEnReserve = new HashSet<>();
        for (Tuile tuile : getTuiles()) {
            if (!tuile.getClass().isInstance(new TuileMer())) {
                Sommet sommet = new Sommet(tuile, this);
                SommetsEnReserve.add(sommet);
                for (Tuile tuileVoisine : tuile.getVoisines()) {
                    if (!tuileVoisine.getClass().isInstance(new TuileMer()) && tuiles.indexOf(tuileVoisine) < tuiles.indexOf(tuile)) {
                        for (Sommet s : SommetsEnReserve) {
                            if (s.getIndice() == tuiles.indexOf(tuileVoisine)) {
                                sommet.ajouterVoisin(s);
                            }
                        }
                    }
        ...
        Set<Sommet> listSommets = new HashSet<>(SommetsEnReserve);
        return new Graphe(listSommets);
    }
