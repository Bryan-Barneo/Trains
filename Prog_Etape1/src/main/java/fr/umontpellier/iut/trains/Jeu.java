...
public class Jeu implements Runnable {
    ...
    /**
     * Booleen pour savoir si l'on est oui ou non premier tour(ou initialisation) du jeu
     */
    ...
    public void setNbJetonsGare(int nb) {
        this.nbJetonsGare += nb;
    }
    ...
    public void run() {
        ...
        List<Integer> PointVictoire = new ArrayList<Integer>();
        for (Joueur j : joueurs) {
            log(String.format("%s : %d points", j.toLog(), j.getScoreTotal()));
            PointVictoire.add(j.getScoreTotal());
        }
        int max = Collections.max(PointVictoire);
        for (Joueur j : joueurs) {
            if (j.getScoreTotal() == max) {
                log(String.format("Le gagnant est %s avec %d points", j.getNom(), j.getScoreTotal()));
            }
        }
    ...
    public boolean estFini() {
        ...
        int pileVide = 0;
        for (ListeDeCartes p : reserve.values()) {
            if (p.isEmpty()) {
                pileVide++;
            }
        }
